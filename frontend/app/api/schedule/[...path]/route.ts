...
/**
 * The Laravel paths this route may reach, anchored so nothing outside the schedule
 * module can be called through it. It is a separate gate from the academic one on
...
/** Forwards a read request for one schedule resource. */
...
/** Forwards a create request for one schedule resource. */
...
/** Forwards a full-replacement update for one schedule resource. */
export async function PUT(request: NextRequest, context: RouteContext<"/api/schedule/[...path]">) {
  const { path } = await context.params;

  return forward(request, path, "PUT");
}

/** Forwards a partial update for one schedule resource. */
export async function PATCH(request: NextRequest, context: RouteContext<"/api/schedule/[...path]">) {
  const { path } = await context.params;

  return forward(request, path, "PATCH");
}

/** Forwards a delete request for one schedule resource. */
export async function DELETE(
  request: NextRequest,
  context: RouteContext<"/api/schedule/[...path]">,
) {
  const { path } = await context.params;
