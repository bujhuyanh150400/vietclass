...
function fileActions(file: ManagedFile) {
  return {
    canTrash: file.trashed_at === null && !file.is_linked,
    canRestore: file.trashed_at !== null,
    canDeletePermanently: file.trashed_at !== null && !file.is_linked,
    canPreview: file.category === "image" || file.category === "pdf",
  };
}
...
function FileMenu({
  ...
  onAction: (
    kind: "rename" | "trash" | "restore" | "permanent",
    file: ManagedFile,
  ) => void;
}) {
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <AppButton
          variant="ghost"
          size="icon-sm"
          aria-label={`Thao tác với ${file.display_name}`}
        >
      ...
      <DropdownMenuContent align="end">
        ...
        <DropdownMenuItem asChild>
          <a href={fileContentUrl(file.id, true)}>
            <Download aria-hidden="true" />
            Tải xuống
          </a>
        </DropdownMenuItem>
        {file.trashed_at === null ? (
          <DropdownMenuItem onSelect={() => onAction("rename", file)}>
          ...
          <>
            ...
            {actions.canTrash ? (
              ...
              <DropdownMenuItem disabled>
                <Trash2 aria-hidden="true" />
                Chuyển vào thùng rác (đang dùng)
              </DropdownMenuItem>
            )}
          </>
        ) : null}
        {actions.canRestore ? (
          <DropdownMenuItem onSelect={() => onAction("restore", file)}>
            <RotateCcw aria-hidden="true" />
            Khôi phục
          </DropdownMenuItem>
        ) : null}
        {actions.canDeletePermanently ? (
          <DropdownMenuItem
            variant="destructive"
            onSelect={() => onAction("permanent", file)}
          >
            <Trash2 aria-hidden="true" />
            Xóa vĩnh viễn
          </DropdownMenuItem>
...
export function FileList({
  ...
  const columns: ListTableColumn<ManagedFile>[] = [
    {
      key: "name",
      header: "Tệp",
      width: 32,
    ...
    },
    {
      ...
      header: "Dung lượng",
      width: 12,
      cell: (file) => formatFileSize(file.size_bytes),
    },
    {
      key: "owner",
      header: "Chủ sở hữu",
      width: 16,
      cell: (file) => file.owner.display_name,
    },
    {
      key: "usage",
      header: "Sử dụng",
      ...
      cell: (file) =>
        file.is_linked ? (
          <div className="grid gap-1">
            <InlineBadge type="primary" className="font-sans">
              Đang dùng
            </InlineBadge>
            <span className="text-xs text-muted-foreground">
              Đang được sử dụng nên không thể chuyển vào thùng rác.
            </span>
          </div>
        ) : (
          "—"
    ...
    },
    {
      ...
      width: 4,
      cell: (file) => (
        <FileMenu file={file} onPreview={onPreview} onAction={onAction} />
      ),
    },
  ];
  ...
  return (
    <>
      ...
      <div className="grid gap-3 md:hidden">
        <MobileFileList state={state} onPreview={onPreview} onAction={onAction} />
      </div>
    </>
  );
}
          ...
          <CardContent className="grid gap-3">
            ...
            {file.is_linked ? (
              <p className="text-sm text-muted-foreground">
                Tệp đang được sử dụng nên không thể chuyển vào thùng rác.
              </p>
            ) : null}
          </CardContent>
        </Card>
