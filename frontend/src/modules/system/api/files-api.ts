...
export async function fetchFiles(params: FileListParams): Promise<Page<ManagedFile>> {
  return browserRequestList<ManagedFile>("/api/v1/system/files", { params });
}

/** Fetches safe metadata for one visible file. */
export async function fetchFile(id: number): Promise<ManagedFile> {
  return browserRequest<ManagedFile>(`/api/v1/system/files/${id}`);
}
...
export async function fetchFileOwnerOptions(
  ...
  return browserRequest<FileOwnerOption[]>("/api/v1/system/files/owner-options", { params });
}
...
export async function fetchFileUsage(
  ...
  return browserRequest<FileUsage>("/api/v1/system/files/usage", { params });
}
...
export async function uploadFile(
  ...
  return browserRequest<ManagedFile>("/api/v1/system/files", {
    method: "POST",
    body,
    signal,
...
export function fileContentUrl(id: number, download = false): string {
  return browserApiUrl(`/api/v1/system/files/${id}/content${download ? "?download=1" : ""}`);
}
...
export async function updateProfileAvatar(
  ...
  return browserRequest<AvatarValue>(`/api/v1/academic/profiles/${profileId}/avatar`, {
    method: "PUT",
    body: avatar,
  });
}
