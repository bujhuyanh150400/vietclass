...
- Đổi ảnh đại diện luôn là một endpoint riêng, độc lập với endpoint sửa hồ sơ. Nút bấm thì tùy màn: màn `/account/avatar` và màn sửa giáo viên có nút **Lưu ảnh đại diện** riêng, còn màn sửa học sinh gộp vào nút **Lưu thay đổi** của tờ hồ sơ và ghi ảnh sau khi các trường hồ sơ được chấp nhận.
...
- Chỉ một bộ hình DiceBear được cài sẵn; thêm kiểu khác phải sửa đồng thời ba nơi gương nhau: `api/config/avatar.php`, `frontend/src/modules/avatar/schemas/avatar-schema.ts` và bảng `STYLES` trong `frontend/src/modules/avatar/utils/dicebear.ts`.
...
- Danh sách tùy chọn DiceBear được phép: `api/config/avatar.php`, gương ở `frontend/src/modules/avatar/schemas/avatar-schema.ts`
- Map giới tính sang biến thể tóc Adventurer: `frontend/src/modules/avatar/utils/adventurer.ts`
