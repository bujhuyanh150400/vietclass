...
Identity cung cấp đăng nhập bằng username và mật khẩu, sau đó cấp bearer token để gọi các API cần xác thực. Chức năng này có hai mặt sử dụng:
...
| --- | --- | --- | --- |
| API cần `auth:sanctum` | Tiên quyết | Token Identity xác định người dùng của request. | Không có bearer token hợp lệ thì không truy cập được endpoint cần xác thực. |
...
- Giá trị role được trả về và được hiển thị dưới dạng nhãn, nhưng chưa có middleware hay cơ chế phân quyền theo role. Mọi người dùng đã đăng nhập đều thấy cùng một khu vực đã đăng nhập.
...
- Route module Laravel: `api/app/Modules/Identity/Routes/api.php`
