...
Chức năng dùng được cả qua API lẫn màn hình quản trị tại `/system/files` (mục **Quản lý thư viện** trong nhóm Hệ thống ở thanh bên).
...
| Thao tác | Yêu cầu |
| --- | --- |
| Xem danh sách | `GET /api/v1/system/files` |
| Xem dung lượng đã dùng | `GET /api/v1/system/files/usage` |
| Xem danh sách chủ sở hữu chọn được | `GET /api/v1/system/files/owner-options`, tùy chọn `q` và `limit` (tối đa 50) — chỉ Quản trị viên, vai trò khác nhận `403` |
| Tải lên | `POST /api/v1/system/files` dạng multipart với `file`, tùy chọn `display_name` và `owner_user_id` |
| Xem chi tiết | `GET /api/v1/system/files/{id}` |
| Đổi tên hiển thị | `PUT /api/v1/system/files/{id}` với `display_name` |
| Mở nội dung | `GET /api/v1/system/files/{id}/content`, tùy chọn `download=1` |
| Chuyển vào thùng rác | `DELETE /api/v1/system/files/{id}` |
| Khôi phục | `POST /api/v1/system/files/{id}/restore` |
| Xóa vĩnh viễn | `DELETE /api/v1/system/files/{id}/permanent` |
| Xem hạn mức | `GET /api/v1/system/settings/file-quotas` |
| Sửa hạn mức | `PUT /api/v1/system/settings/file-quotas` với `quotas.admin`, `quotas.teacher`, `quotas.student`, `quotas.guardian` tính theo byte |
...
Trên trình duyệt tại `/system/files`:
...
| [Ảnh đại diện hồ sơ](../academic/avatar.md) | Hạ nguồn | Ảnh được chọn làm ảnh đại diện tạo một liên kết chặn chuyển vào thùng rác và xóa vĩnh viễn. | Ảnh đang dùng làm ảnh đại diện không xóa được cho tới khi đổi ảnh đại diện. |
...
- Route: `api/app/Modules/System/Routes/api.php`
- Tác vụ định kỳ: `api/app/Modules/System/Routes/console.php` (`files:purge-trash`, chạy hằng ngày)
- Cấu hình kho lưu trữ: `api/config/system.php`, `api/config/filesystems.php`
- Kiểm thử xác định: các bài kiểm thử file trong `api/tests/Behavioral/` và `api/tests/Security/`
