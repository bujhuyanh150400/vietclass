...
test('the subject list is paginated with the shared meta envelope', function () {
    ...
    $this->getJson('/api/v1/academic/subjects?per_page=2')
        ->assertOk()
...
test('the subject list rejects paging and sorting values outside the contract', function () {
    $this->getJson('/api/v1/academic/subjects?per_page=500')->assertJsonValidationErrorFor('per_page');
    $this->getJson('/api/v1/academic/subjects?page=0')->assertJsonValidationErrorFor('page');
    $this->getJson('/api/v1/academic/subjects?sort=password')->assertJsonValidationErrorFor('sort');
    $this->getJson('/api/v1/academic/subjects?direction=sideways')->assertJsonValidationErrorFor('direction');
});
...
test('the subject list searches by name and filters by locked state', function () {
    ...
    $this->getJson('/api/v1/academic/subjects?q=to%C3%A1n')
        ->assertOk()
        ->assertJsonPath('meta.total', 2);

    $this->getJson('/api/v1/academic/subjects?is_active=0')
        ->assertOk()
...
test('the subject list filters by applicable grade and sorts by running class count', function () {
    ...
    $this->getJson('/api/v1/academic/subjects?grade_level='.GradeLevel::Grade9->value)
        ->assertOk()
        ->assertJsonPath('meta.total', 2);

    $this->getJson('/api/v1/academic/subjects?sort=active_classes_count&direction=desc')
        ->assertOk()
...
test('a literal wildcard in the search term matches itself', function () {
    ...
    $this->getJson('/api/v1/academic/subjects?q=100%25')
        ->assertOk()
        ->assertJsonPath('meta.total', 1);
});
...
test('a duplicate subject name is reported against the name field', function () {
    ...
    $this->postJson('/api/v1/academic/subjects', ['name' => 'Hóa học'])
...
test('a subject is locked and unlocked through its own endpoint', function () {
    ...
    $this->patchJson("/api/v1/academic/subjects/{$subject->id}/active", ['is_active' => false])
        ->assertOk()
        ->assertJsonPath('data.is_active', false);

    $this->patchJson("/api/v1/academic/subjects/{$subject->id}/active", ['is_active' => true])
        ->assertOk()
        ->assertJsonPath('data.is_active', true);
});
...
test('a subject taught by a running class cannot be locked', function () {
    ...
    $this->patchJson("/api/v1/academic/subjects/{$class->subject_id}/active", ['is_active' => false])
        ->assertStatus(409)
...
test('a subject no class references is removed', function () {
    ...
    $this->deleteJson("/api/v1/academic/subjects/{$subject->id}")->assertNoContent();
...
test('a missing subject is reported as not found by every operation', function () {
    ...
    $this->getJson('/api/v1/academic/subjects/9999')
        ->assertNotFound()
...
test('the subject option list offers only unlocked subjects', function () {
    ...
    $this->getJson('/api/v1/academic/subjects/options')
        ->assertOk()
