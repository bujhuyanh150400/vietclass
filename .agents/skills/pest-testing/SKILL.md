...
All tests must be written using Pest. Use `php artisan make:test --pest {name}`, then place the file in the semantic suite that owns it.

The Laravel generator still chooses its own default directory. Move the generated file into `tests/Behavioral/...` or `tests/Security/...` immediately, and do not leave active tests in a third suite root.

### Test Organization

- Behavioral tests: `tests/Behavioral/` (pure tests may remain framework-free; application-backed tests belong under its `Api/` subtree).
- Security tests: `tests/Security/`.
- Browser tests belong under `Behavioral` or `Security` according to the behavior they protect; do not create a third root.
...
- Run file: `php artisan test --compact tests/Behavioral/ExampleTest.php`.
...
- Browser tests live under the owning `tests/Behavioral/` or `tests/Security/` subtree.
